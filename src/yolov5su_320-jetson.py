...
model_path = "yolov5su_saved_model_320"
...
def on_frame(sink, appsrc, interpreter, labels, input_details, output_details, input_size, scale_out, zero_point_out, scale_in, zero_point_in, last_time):
    ...
    if sample:
        ...
        try:
            ...
            input_data = (img_resized / 255 / scale_in + zero_point_in)
            input_data = tf.convert_to_tensor(np.expand_dims(input_data, axis=0).astype(np.float32))
            ...
            detections = output[output_name].numpy()[0].T.astype("float32")
