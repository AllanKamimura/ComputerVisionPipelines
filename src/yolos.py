...
MINIMUM_SCORE = float(os.environ.get("MINIMUM_SCORE", 0.15))
...
INFERENCE_SIZE = (1062, 800)  # Smaller size for faster inference

model_path = "yolos_quant.tflite"
...
def on_frame(sink, appsrc, interpreter, labels, input_details, output_details, input_size, scale, zero_point, last_time):
    ...
    if sample:
        ...
        try:
            ...
            t2 = time()
            print(t2-t1)
            detections = interpreter.get_tensor(output_details[0]['index'])[0].astype("float32")  # Shape: [25200, 85]
            detections -= zero_point
            detections *= scale
            print(detection)
