...
# from tflite_runtime.interpreter import Interpreter, Delegate
...
def on_frame(sink, appsrc, interpreter, labels, input_details, output_details, input_size, last_time):
    ...
    if sample:
        ...
        try:
            ...
            detections = interpreter.get_tensor(output_details[3]['index'])[0]
            ...
            # Display inference time with yellow text and black outline for better contrast
            cv2.putText(
                img_array, f"Inference time: {t2 - t1:.3f}s", (5, 15),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA  # Black outline
            )
            cv2.putText(
                img_array, f"Inference time: {t2 - t1:.3f}s", (5, 15),
            ...
            # Display FPS with yellow text and black outline for better contrast
            cv2.putText(
